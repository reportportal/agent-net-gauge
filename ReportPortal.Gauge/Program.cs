...
namespace ReportPortal.GaugePlugin
{
    class Program
    {
        ...
        {
            var portEnvVar = Environment.GetEnvironmentVariable("plugin_connection_port");
            TraceLogger.Verbose($"plugin_connection_port environment variable: {portEnvVar}");

            var port = Convert.ToInt32(portEnvVar);
            TraceLogger.Verbose($"Plugin connection port: {port}");
